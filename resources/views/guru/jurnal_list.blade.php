...
<body class="bg-brand-50 font-sans min-h-screen flex flex-col justify-between text-[#3E3028]">
  ...
  <div class="w-full flex-1 flex flex-col items-center pb-28 sm:pb-32">
    ...
    <header class="w-full bg-white border-b border-brand-100 sticky top-0 z-40 shadow-sm">
      <div class="max-w-5xl mx-auto px-4 h-14 flex items-center justify-between">
        <a href="{{ url('/dashboard-guru') }}" class="w-9 h-9 flex items-center justify-center rounded-xl bg-brand-50 hover:bg-brand-100 text-[#3E3028] transition-all active:scale-95" aria-label="Kembali">
          <svg class="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
            <path d="M15 18l-6-6 6-6"/>
          </svg>
        </a>
        <h1 class="font-poppins font-bold text-base sm:text-lg text-[#3E3028]">Jurnal Mengajar</h1>
        <div class="w-9"></div>
      </div>
    </header>

    <!-- Main Content Container -->
    <main class="w-full max-w-5xl px-4 sm:px-6 py-6 sm:py-8 flex flex-col gap-5">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div class="bg-white border border-brand-100 rounded-2xl p-5 shadow-xs hover:shadow-md transition-shadow flex flex-col justify-between gap-4">
          ...
          <div class="flex flex-col gap-3">
            <!-- Header Card: Mapel, Kelas, & Status -->
          ...
          <div class="flex justify-end gap-2 pt-2 border-t border-brand-50">
            ...
            <a href="{{ url('/ringkasan-sesi') }}" class="px-3.5 py-1.5 bg-brand-800 hover:bg-brand-900 text-white font-poppins font-semibold text-xs rounded-lg shadow-xs transition-colors">
              Lihat
            </a>
          </div>
  ...
  <nav class="fixed bottom-0 left-0 right-0 bg-white border-t border-brand-100 py-2.5 px-6 sm:px-10 z-50 shadow-[0_-4px_20px_rgba(0,0,0,0.05)]">
    <div class="max-w-7xl mx-auto flex justify-between md:justify-center md:gap-32 items-center h-12">
      ...
      <a href="{{ url('/riwayat-jurnal') }}" class="flex flex-col md:flex-row items-center gap-1 md:gap-2.5 text-xs md:text-sm font-bold text-brand-800">
