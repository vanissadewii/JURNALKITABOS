...
<body class="bg-brand-50 font-sans min-h-screen flex text-[#3E3028]">
  ...
  <div class="flex-1 md:ml-64 flex flex-col min-h-screen pb-24 md:pb-8">
    ...
    <main class="w-full max-w-7xl mx-auto px-4 sm:px-6 md:px-8 py-6 sm:py-8 flex-1 flex flex-col items-center gap-6">
      ...
      <!-- Form Data Profil -->
      <form action="{{ route('profil-guru.update') }}" method="POST" class="w-full flex flex-col gap-6">
        @csrf
        ...
        <div class="bg-white border border-brand-100 rounded-2xl p-6 sm:p-8 shadow-xs flex flex-col gap-5 w-full">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
            ...
            <div class="flex flex-col gap-1.5">
              <label for="no_telepon" class="text-xs font-semibold text-brand-600">No. Handphone</label>
        ...
        <!-- Password Card -->
        <div class="bg-white border border-brand-100 rounded-2xl p-6 sm:p-8 shadow-xs flex flex-col gap-5 w-full">
          <h2 class="font-poppins font-bold text-base sm:text-lg text-[#3E3028] border-b border-brand-100 pb-3">Ubah Password <span class="text-xs font-medium text-brand-600">(kosongkan jika tidak diganti)</span></h2>

          <div class="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-6">
            @foreach ([
            ['passwordLama', 'password_lama', 'Password Lama', 'Masukkan password lama'],
            ['passwordBaru', 'password_baru', 'Password Baru', 'Minimal 8 karakter'],
            ['konfirmasiPassword', 'password_baru_confirmation', 'Konfirmasi Password Baru', 'Ulangi password baru'],
            ] as [$id, $nama, $label, $placeholder])
            <div class="flex flex-col gap-1.5">
              <label for="{{ $id }}" class="text-xs font-semibold text-brand-600">{{ $label }}</label>
              <div class="relative w-full">
                <input type="password" id="{{ $id }}" name="{{ $nama }}" placeholder="{{ $placeholder }}" autocomplete="new-password"
